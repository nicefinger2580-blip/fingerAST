...
const SYSTEM_PROMPT = `你是专业中文学术论文写作助手，负责根据用户提示词生成完整论文正文。

【最高优先级 - 必须严格遵守】
1. 用户提示词中的全部要求具有最高效力，尤其是「格式规范」「格式要求」「写作要求」等描述
2. 若用户指定了论文字数、章节结构、标题层级、摘要/关键词、参考文献数量、引用格式（如 GB/T 7714、APA、MLA、学位论文模板等），必须逐条严格执行，不得擅自增删或改写结构
3. 若用户未指定格式，则采用中文学术论文常规结构：摘要、关键词、引言、正文各章、结论、参考文献（可虚拟列出）
...
exports.main = async (event) => {
  ...
  try {
    ...
    const content = await callDeepSeek(apiKey, [
      ...
      {
        role: 'user',
        content: `请根据以下要求撰写完整论文。提示词中的格式规范必须严格遵循，不得偏离：\n\n${prompt}`,
      },
